...
public class DataLogicSales extends BeanFactoryDataSingle {
    ...
    public final SentenceList getTaxList() {
        return new StaticSentence(s, "SELECT ID, NAME, CATEGORY, CUSTCATEGORY, PARENTID, RATE, RATECASCADE, RATEORDER FROM TAXES WHERE ID  NOT LIKE '-1' ORDER BY NAME", null, new SerializerRead() {
    ...
    public final SentenceList getCategoriesList() {
        return new StaticSentence(s, "SELECT ID, NAME, IMAGE FROM CATEGORIES WHERE ID  NOT LIKE '-1'  ORDER BY NAME", null, CategoryInfo.getSerializerRead());
    }
    ...
    public final SentenceList getTaxCategoriesList() {
        return new StaticSentence(s, "SELECT ID, NAME FROM TAXCATEGORIES WHERE ID  NOT LIKE '-1' ORDER BY NAME", null, new SerializerRead() {
    ...
    public final TicketInfo loadTicket(final int tickettype, final int ticketid) throws BasicException {
        ...
        if (ticket != null) {
            ...
            ticket.setLines(new PreparedSentence(s, "SELECT L.TICKET, L.LINE, L.PRODUCT, L.ATTRIBUTESETINSTANCE_ID, L.UNITS, L.PRICE, T.ID, T.NAME, T.CATEGORY, T.CUSTCATEGORY, T.PARENTID, T.RATE, T.RATECASCADE, T.RATEORDER, L.ATTRIBUTES "
    ...
    public final void saveSupplierPayment(final TicketInfo ticket, final String location) throws BasicException {
        ...
        Transaction t = new Transaction(s) {
            ...
            public Object transact() throws BasicException {
                ...
                SentenceExec paymentinsert = new PreparedSentence(s, "INSERT INTO PAYMENTS (ID, RECEIPT, PAYMENT, TOTAL, TRANSID, RETURNMSG, CHEQUENUMBER, BANKNAME, CHEQUEDATE, SUPPLIER ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", SerializerWriteParams.INSTANCE);
                for (final PaymentInfo p : ticket.getPayments()) {
                    // MSL : set bank name to string
